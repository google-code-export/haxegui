...
class Stepper extends Component, implements Dynamic
{
  ...
  public function init(?initObj:Dynamic)
	...
	//~ back.graphics.drawRoundRect(0,0,40,30,8,8);
	back.graphics.drawRoundRectComplex (0, 0, 40, 30, 4, 0, 4, 0 );
	...
	up.init({width: 15, height: 15});
	up.removeChild(up.label);
	up.move( 40, 0 );
	
	down = new Button(this, "down");
	down.init({width: 15, height: 15});
	down.removeChild(down.label);
	down.move( 40, 15 );

	// add the drop-shadow filter
	shadow = new DropShadowFilter (4, 45, StyleManager.DROPSHADOW, 0.8, 4, 4, 0.65, BitmapFilterQuality.HIGH, false, false, false );
	up.filters = [shadow];
	down.filters = [shadow];
